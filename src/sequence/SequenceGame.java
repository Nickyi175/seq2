...
public char[][] board;
	...
	public SequenceGame(int numberOfPlayers) throws InterruptedException {
        ...
        isStartingNewGame = JOptionPane.showConfirmDialog(gui, "¿Seguir jugando?", 
	...
	void createPlayers(int n) {
		...
		if(n<1)
			System.out.println("Necesitas mas de un jugador");
		else if(n==1) {
	...
	void startGame(int numberOfPlayers) throws InterruptedException {
		...
		case 1:
			...
			//assign player name
			String name = JOptionPane.showInputDialog(gui, "Ingresa tu usuario ", "Nicole");
			...
			if(name=="" || isBlank) {
				//set default name
				human.playerName = "Nicole";
				//notify user
				JOptionPane.showMessageDialog(gui, "Nombre de prueba: Nicole", "",
			...
			JOptionPane.showMessageDialog(
			  gui, list, "Elige el color de tu ficha", JOptionPane.PLAIN_MESSAGE);
			
			//player did NOT select a color
			while(list.getSelectedIndex()==-1) {
				JOptionPane.showMessageDialog(gui, "No haz seleccionado un color aún", "Cuidado", JOptionPane.ERROR_MESSAGE);
			...
			//log players' info
			String playerInfo = "Usuario " + human.playerName + "(" + human.playerColor + ")\n"
					+ "Color contrincante" + cpu.playerColor + "\n";
			...
			if(p0 instanceof HumanSequencePlayer) {
				...
				while(true) {
					if(isGameOver(lastPlayedX, lastPlayedY)) {
						...
						currentPlayer = human;
						ColorJugadorActual = human.playerColor;
						human.enableAllHandCards();
						
						//log human's hand
						log.updateLog("Turno predeterminado " + turn + ":"
								+ "\n\tBaraja actual " + currentPlayer.getHand());
					...
					//computer's turn
					currentPlayer = cpu;
					ColorJugadorActual = cpu.playerColor;

					//log cpu's hand
					log.updateLog("turno 2" + turn + ":"
							+ "\n\tBaraja actual " + currentPlayer.getHand());
...
//end of else	
	...
	void endGame() {
		...
		log.updateLog("\tMovimiento: " + lastPlayedCard.getCardName() +
				" at " + "(" + lastPlayedX + ", " + lastPlayedY + ")");
		log.updateLog("\nSe terminó el juego en el turno " + turn + "\nGanador es " + winner
				+ ", por " + winBy);
	}
